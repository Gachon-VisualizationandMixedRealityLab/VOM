function loaded() {
    ...
    //데이터 CSV 불러오기
    const fileName = "/[repo]/assets/csv/demo.csv";
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var num = 0; num < rowArr.length; num++) {
                ...
                if (textArr[num][0] == "1" && num > 0) {

                    //데이터 받아오기!!
                    const title = textArr[num][1];
                    const youtubelink = textArr[num][2];
                    const text = textArr[num][3];
                    const paper =  textArr[num][4];

                    switch (textArr[num][0]) {                                           
                        default:
                            ul_list = $("#demo_content");
                            break;
                    }
                                      //여기다가 html 짜서 넣는거구나
                    ul_list.append(`<div class="row">
                                        ...
                                        <iframe src="https://www.youtube.com/embed/${youtubelink}" title="YouTube video player"
                                            frameborder="0"
